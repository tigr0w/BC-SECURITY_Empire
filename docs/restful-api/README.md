...
_/api/v2/tags_ Tags are a flat name with a single shared color and an optional description, defined once in a global registry and attached to many entities. The same tag always resolves to the same row — and therefore the same color — wherever it is applied. Taggable entities are listeners, agents, agent tasks, plugin tasks, credentials, and downloads.

The registry endpoints manage tags independent of any entity:

* `GET /api/v2/tags` lists tags (paged, ordered, filterable by `query` and `sources`). Each row includes a `usage_count` — the number of entities the tag is attached to across every taggable type.
* `GET /api/v2/tags/{id}` returns a single tag.
* `POST /api/v2/tags` creates a tag from `{name, color?, description?}`. If `color` is omitted, a stable color is derived from the name, so a new tag immediately gets a distinct, consistent color (editable afterward). A duplicate name returns `409`.
* `PUT /api/v2/tags/{id}` renames, recolors, or edits the description. Omitted fields are left unchanged; a name collision returns `409`.
* `DELETE /api/v2/tags/{id}` removes the tag and detaches it from every entity.

Each taggable entity exposes attach/detach sub-resources, e.g. `/api/v2/agents/{id}/tags`:

* `POST {entity}/{id}/tags` attaches an existing tag by id: `{tag_id}`. Create the tag first via `POST /api/v2/tags`. Attaching a tag the entity already carries is an idempotent `200 OK`; an unknown `tag_id` returns `404`.
* `DELETE {entity}/{id}/tags/{tag_id}` detaches the tag from that entity. The tag row itself persists in the registry until it is explicitly deleted, even after its last entity is detached.

List endpoints that accept a `tags` query parameter (agent tasks, plugin tasks, credentials, downloads) filter by **exact** tag name (e.g. `?tags=production`); pass the parameter multiple times to match any of several tags.
