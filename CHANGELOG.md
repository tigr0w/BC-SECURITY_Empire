...
-   Replaced MD5 with SHA-256 for staging key normalization (non-32-char keys) per FIPS. **Breaking:** agents staged under the old algorithm must be re-staged.
...
-   Replaced `random.choice`/`random.choices` with `secrets.choice` (CSPRNG) for staging key, keyword obfuscation, session ID, and nonce generation per FIPS SP 800-90A.
-   Hardened TLS for FIPS SP 800-52r2: enforce TLS 1.2 minimum, drop non-GCM/non-ECDHE cipher suites from the JA3 evasion pool, and use an explicit RSA 4096-bit key for self-signed certificates.
-   Replaced all remaining `random` usage with `secrets` (server) and `random.SystemRandom()` (agent) for FIPS SP 800-90A across listeners, stagers, agents, and utilities. **Breaking:** deployed agents must be re-staged.
...
-   Malleable profile `header "Name" "Value";` directives now reject CRLF in the name or value at **both** parse and deserialize time (dropping the directive with a WARNING), closing a header-injection vector through the Flask `after_request` merge. Both call sites share a `_header_pair_is_safe` helper.
...
-   Added `reg_query` BOF module (`bof/situational_awareness/reg_query`) wrapping the TrustedSec reg_query BOF for in-agent Windows registry queries on local or remote systems. Supports key enumeration, value lookup, and recursive traversal. Refreshed in-repo `.o` to the current upstream build.
...
-   Added a pure-Python AES-256-GCM implementation (`agent/stagers/common/aesgcm.py`) for Python/IronPython stagers, compatible with .NET Framework 4.x IronPython.
-   Added a .NET Framework 4.x compatible C# `AesGcmHelper` class (manual GCM via `AesCryptoServiceProvider` ECB + GHASH/GCTR) for PowerShell agent stagers.
-   Added cross-implementation interoperability tests verifying server and agent AES-GCM produce identical output.
-   Added routing-packet tests for wrong-key rejection, tampered packets, AAD mismatch, empty/non-block-aligned plaintexts, and concatenated packets.
...
-   **BREAKING:** Base directories now use `platformdirs` for XDG-compliant (Linux) and native (macOS/Windows) paths. On Linux with `$XDG_*_HOME` unset, `~/.config/empire` and `~/.local/share/empire` are unchanged (no-op). The Go build cache moved from `~/.local/share/empire/.cache/go-build` to the platform cache dir (`~/.cache/empire/go-build` on Linux, honoring `$XDG_CACHE_HOME`) and simply rebuilds. macOS/Windows operators get correct native paths (`~/Library/Application Support/empire`, `%LOCALAPPDATA%\empire`) for the first time; prior `~/.local/share/empire` state needs a one-time manual move. Linux operators who export `$XDG_DATA_HOME`/`$XDG_CONFIG_HOME` will see their data/config root relocate accordingly.
...
-   **BREAKING:** Module option metadata changes shape in GET responses for typed options. Boolean options were migrated to native `value: false` in module YAML, so `EmpireModuleOption` now infers `type` and a typed option reports a real `value_type` (`BOOLEAN`/`INTEGER`/`FLOAT` instead of `STRING`), `type` is populated (e.g. `bool`), boolean switches dropped their `strict: true` + `suggested_values: ['True', 'False']` scaffolding, and 14 boolean options were standardized from `required: true` to `required: false`. Frontends can use `value_type` to render typed inputs (toggles, number fields), but clients that consume option metadata should expect these new typed fields. The module task POST body (`ModulePostRequest.options`) stays string-coerced like stagers, listeners, and plugins — native JSON `bool`/`int`/`float` are accepted on the wire and normalized to strings, then re-typed per option by `safe_cast` — so POST clients are unaffected.
-   Extended typed boolean options to stager and listener templates. Stager GET template responses now report `value_type: BOOLEAN` for boolean options (e.g. `Obfuscate`, `Base64`, `Delete`, `Staged`) and listener GET template responses report `value_type: BOOLEAN` for `JA3_Evasion`. Boolean options in stager definitions were migrated from string `"True"`/`"False"` defaults with `strict`/`suggested_values` scaffolding to native `true`/`false`. `JA3_Evasion` in `http` and `http_malleable` listeners was also migrated, fixing a latent bug where the string default `'False'` was always truthy. POST bodies are unaffected — native JSON bools continue to be accepted on the wire. Stager and listener bool options now flow as native Python booleans end-to-end through generation.
-   Migrated server-side module `generate()` methods to read native typed options directly (`if params["X"]:` instead of `params["X"].lower() == "true"`) and removed the internal `normalize_legacy_params` re-stringify shim from both `_generate_script` and `validate_options`. The string-building paths that genuinely need text — the non-custom PowerShell/Python/C# templates and modules' own `params.items()` loops — now stringify primitives at point of use. The module/stager/listener/plugin POST bodies stay string-coerced via `coerced_dict`, so API clients are unaffected; the only behavior dropped is for **direct (non-API) callers** of `validate_options` passing a native `bool` against a legacy stringly `depends_on` / `suggested_values` list (the API path already coerces these to `"True"`/`"False"`).
...
-   Updated Empire Compiler to v1.1.0-a.5 (FIPS-compliant Sharpire with 16-byte HMAC, HKDF-SHA256, AES-GCM).
...
-   Updated C# module YAMLs to the new Empire Compiler format.
...
-   Demoted the malleable "accepting `set <key>` but Empire does not act on it yet" log from INFO to DEBUG (the allow-list spans 60+ keys and flooded consoles); genuinely-unknown directives still WARN.
...
-   Fixed `bof/credentials/nanodump` boolean flags (`valid`, `fork`, `snapshot`, `duplicate`, `getpid`, etc.) being silently ignored: the module compared the option value against lowercase `"true"` while boolean option values render capitalized as `"True"`, so every flag resolved to `0`. They now read the native bool directly.
...
-   Fixed the dead `length < 0` bounds check in agent `parse_routing_packet`, replaced with proper `end > len(data)` validation.
...
-   Fixed BOF argument packing in `generate_script_bof` and `generate_go_bof`: empty optional `z`-format options were replaced with a space (`" "`) sentinel that BOFs checking `*arg == '\0'` for "no value given" would misinterpret as a real string (e.g. `reg_query` called `RegConnectRegistryA(" ", ...)` → error 53 / `ERROR_BAD_NETPATH`). Registry paths without spaces also had backslashes silently stripped by POSIX-mode `shlex`. Both functions now pass arguments as a pre-built list to `process_arguments`, bypassing `shlex` entirely; `process_arguments` accepts `list | str` so the string path remains for callers that still use it.
-   Fixed C# / IronPython / Go stagers being blocked against the malleable HTTP listener even though Sharpire and Gopire support it (commit `09e5f2ad`). Two changes restore end-to-end staging:
    1.  **Sweep the 15 allow-list gates.** `multi/launcher`, `windows/{dll,wmic,hta,ducky,war,macro,launcher_xml,launcher_vbs,teensy,cmd_exec,bunny}`, and `powershell/lateral_movement/invoke_psexec` previously gated payload generation behind a hardcoded `["HTTP[S]", "smb_pivot", "port_forward_pivot"]` allow-list and returned an empty launcher (or raised `ModuleValidationException`) when an operator selected `Listener=<malleable> Language=csharp` (or `go` / `ironpython`). Replaced with `stagergenv2.generate_exe_oneliner_routed` / `generate_go_exe_oneliner_routed`, which dispatch via the new `listener.stager_url()` method to the malleable listener's http-stager URI instead of the legacy POST URI extracted from `DefaultProfile` — a POST URI would have routed stage 0 into the listener's POST handler instead of returning the binary.
    2.  **Serve the binary at the malleable stager URI.** `http_malleable.py:generate_stager(csharp/go)` previously returned `""` with a "stageless — binary IS the stage" comment, which was correct for the operator-direct workflow (run the binary on target) but broke the launcher / wrapper flow that downloads bytes from a URL and `Assembly.Load`s (csharp) or `Start-Process`es (go) them. Now mirrors the legacy HTTP listener (`http.py:866-880`) by calling `self.generate_launcher(language)` and returning the binary bytes — so `multi/launcher`, the 11 wrapper stagers (`dll`, `wmic`, `hta`, `ducky`, `war`, `macro`, `launcher_xml`, `launcher_vbs`, `teensy`, `cmd_exec`, `bunny`), and `invoke_psexec` get an executable payload at stage 0 the same way they do against the legacy listener.
-   Fixed boolean module options that were forwarded straight to a truthy check (`if obfuscate:`) being effectively always-on. Removing the `normalize_legacy_params` re-stringify shim (see Changed) lets the native `False` reach custom-generate modules that never converted the legacy string value, so these toggles are finally honored. Most notably `csharp/management/ThreadlessInject`'s `Obfuscate` switch read `params["Obfuscate"]` raw and forwarded the truthy string `"False"` to `generate_launcher`, so its launcher was **always obfuscated regardless of the toggle** — present since the module was introduced (#845) and previously masked by the shim. The same correction applies to the `CheckAll` / `NoDefaults` / `Debug` switches in `powershell/situational_awareness/network/get_sql_server_info`, `powershell/situational_awareness/host/get_sql_column_sample_data`, `python/collection/osx/imessage_dump`, and `python/situational_awareness/host/osx/situational_awareness`.
