...
-   Modernization quick wins (no behavior change): replaced deprecated `datetime.utcnow()` with timezone-aware `datetime.now(UTC)` in `jwt_auth.create_access_token`; swapped five `len(db.query(X).all()) == 0` startup existence checks for O(1) `db.query(X).first() is None` in `core/db/base.py`; dropped the Python-2 `old_div` shim from `helpers.get_file_size` (operands are always ints, so `//` is equivalent) and deleted the now-unused `empire/server/utils/math_util.py` (agent-payload copies under `data/agent/` left untouched); and bumped the `ruff-pre-commit` hook to `v0.15.2` to match the `pyproject.toml` pin.
...
-   Migrated the ORM model definitions in `empire/server/core/db/models.py` from the legacy SQLAlchemy 1.x style (`Column()` + `declarative_base()`) to the 2.0 typed declarative style (`Mapped[]` + `mapped_column()` + `DeclarativeBase`). Schema-preserving: the generated DDL is byte-for-byte identical before and after (verified for the sqlite and mysql dialects, including indexes and the `with_variant(LONGTEXT)` columns), and `alembic` autogenerate detects no diff. Column nullability is now expressed through `Mapped[T]` / `Mapped[T | None]` annotations rather than `nullable=`. Association `Table` constructs, relationship loading strategies, and the `PydanticType` decorator are unchanged.
