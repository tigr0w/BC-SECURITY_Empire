...
-   Replaced `random`/`random.choice` with `secrets`/`secrets.choice` (server) and `random.SystemRandom()` (agent) for CSPRNG generation per FIPS SP 800-90A — staging keys, keyword obfuscation, session IDs, nonces, and all remaining uses across listeners, stagers, agents, and utilities. **Breaking:** deployed agents must be re-staged.
...
-   Smoothed the bcrypt → PBKDF2 upgrade: a bcrypt-shaped default-user hash is auto-reset to PBKDF2 on startup (logged with recovery steps) and non-default bcrypt users are left untouched. **Manual recovery:** an admin `POST /api/v2/users/{id}/password` (or Starkiller) stores a PBKDF2 hash.
-   Added a `./ps-empire update` subcommand that moves the checkout to the latest release tag, refreshes `config.yaml`, and fast-forwards Starkiller, the plugin registry, and the Empire Compiler binary (skips source on dev branches). **BREAKING:** the base `config.yaml` is overwritten on every update — move local customizations to `config.user.yaml`.
...
-   Added AES-256-GCM implementations across the stack: an `AES256GCM` AEAD class in `encryption.py` (server, via `cryptography`), a pure-Python version (`agent/stagers/common/aesgcm.py`) for Python/IronPython stagers, and a .NET 4.x C# `AesGcmHelper` (manual GCM via `AesCryptoServiceProvider` ECB + GHASH/GCTR) for PowerShell stagers.
...
-   Added parser support for the Cobalt Strike `http-config` block (`trust_x_forwarded_for`, `block_useragents`, `header`, `set headers`). The malleable HTTP listener wires `trust_x_forwarded_for` (default False, validated XFF parsing), `block_useragents` (fnmatch globs returning the shared IIS 7.5 404), and `header` (merged into every response); `set headers` ordering is parsed but not yet enforced.
...
-   **BREAKING:** Typed module-option metadata in GET responses. Boolean options migrated to native `value: false` in YAML, so typed options now report a real `value_type` (`BOOLEAN`/`INTEGER`/`FLOAT` instead of `STRING`) and populated `type`; boolean switches dropped their `strict`/`suggested_values` scaffolding and 14 were standardized to `required: false`. POST bodies are unaffected — native JSON `bool`/`int`/`float` are accepted and re-typed per option by `safe_cast`.
...
-   Extended typed boolean options to stager and listener templates. Stager/listener GET responses now report `value_type: BOOLEAN` (e.g. `Obfuscate`, `Base64`, `JA3_Evasion`), migrated from string `"True"`/`"False"` defaults to native bools — also fixing a latent bug where `JA3_Evasion`'s `'False'` default was always truthy. POST bodies are unaffected.
-   Migrated server-side module `generate()` methods to read native typed options (`if params["X"]:`) and removed the `normalize_legacy_params` re-stringify shim. API clients are unaffected (POST bodies stay string-coerced); only direct non-API callers of `validate_options` passing a native `bool` against a legacy stringly `depends_on`/`suggested_values` list see a behavior change.
...
-   Fixed a race condition in concurrent stager generation where `generate_powershell_exe` and `generate_python_exe` shared a single `Data/EmbeddedResources/common/launcher.txt` file, causing cross-contamination of launcher content when two compilations ran simultaneously (e.g. under pytest-xdist). Each call now writes to a unique per-compilation subdirectory and patches the stager YAML `Location` in-memory.
...
-   Fixed `Profile._deserialize`/`HttpsCertificate._deserialize` using raw `bool(...)` for `host_stage`/`trust_x_forwarded_for`, which mapped `"false"` back to `True` (the `host_stage` flip could silently re-enable a disabled stager URI). A `_coerce_bool` helper now mirrors the parse-side vocabulary, and a malformed `validity` is log-dropped instead of killing listener startup.
...
-   Fixed BOF argument packing in `generate_script_bof`/`generate_go_bof`: empty optional `z`-format options were replaced with a `" "` sentinel that BOFs reading `*arg == '\0'` misread as a value (e.g. `reg_query` → `ERROR_BAD_NETPATH`), and POSIX-mode `shlex` stripped backslashes from registry paths. Both now pass arguments as a pre-built list, bypassing `shlex`.
-   Fixed C#/IronPython/Go stagers being blocked against the malleable HTTP listener despite Sharpire/Gopire support. Swept the 15 hardcoded allow-list gates (`multi/launcher`, the `windows/*` wrappers, `invoke_psexec`) to route via the new `listener.stager_url()` to the malleable http-stager URI, and `http_malleable.generate_stager` now serves the binary (like the legacy HTTP listener) instead of returning an empty stage — so launcher/wrapper flows get an executable payload at stage 0.
-   Fixed boolean module options forwarded straight to a truthy check (`if obfuscate:`) being effectively always-on; removing the `normalize_legacy_params` shim lets native `False` reach custom-generate modules. Most notably `csharp/management/ThreadlessInject`'s `Obfuscate` switch was always obfuscating regardless of the toggle; the same fix applies to `CheckAll`/`NoDefaults`/`Debug` switches in several SQL and macOS modules.
...
-   Fixed `ExtFile` mode being non-functional in seven PowerShell persistence modules (`persistence/userland/{registry,schtasks,backdoor_lnk}` and `persistence/elevated/{registry,schtasks,wmi,wmi_updater}`): `enc_powershell()` returns `bytes`, which six concatenated into a `str` script (raising `TypeError`, surfaced to the operator as the generic `Error generating script.`) while `backdoor_lnk` silently embedded the `b'...'` bytes-repr. Each call site now decodes the result to `str`.
...
-   Fixed `is_option_required` raising `KeyError` on a `depends_on` entry that omits a `values` list. It indexed `dependency["values"]` unconditionally, while the sibling `evaluate_dependencies` tolerates a missing `values` key; a valueless dependency now imposes no value constraint (its specific value is simply not checked). No shipped module currently triggers this, but it made any valueless `depends_on` crash module validation.
