...
-   Module options may now carry a per-option `format_string`, overriding the module-wide `advanced.option_format_string` for that one option, plus a new `{{ VALUE_ARRAY }}` token that renders a comma-separated value as individually double-quoted, comma-joined elements. This is what a PowerShell `[Array]` parameter needs: `-NBNSTypes {{ VALUE_ARRAY }}` turns `00,20` into `"00","20"` (`@("00","20")`). See the [PowerShell module docs](https://bc-security.gitbook.io/empire-wiki/module-development/powershell-modules) for usage.
-   New `krb_session_key` credential type, capturing the `Base64(key)` session key Rubeus prints alongside every ticket. Stored as `<keytype>:<base64key>` (e.g. `aes256_cts_hmac_sha1:bsnt...`), since a bare key is ambiguous between `rc4_hmac` and the AES variants and unusable without its encryption type. Emitted for both the `dump` and `asktgt`/`asktgs` output layouts, including legacy 8-byte DES keys (`des_cbc_md5` / `des_cbc_crc`), which are shorter than the modern etypes. `credtype` is a free-form column, so no migration is required.
