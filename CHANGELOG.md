...
-   `header "Name" "Value";` directives in malleable profiles now reject CRLF in either the name or the value at **both** parse time and deserialize time, dropping the directive with a WARNING log naming the source (`parse` vs `deserialize`). Without this, a malicious or operator-pasted profile that smuggled `\r\n` into a header — or a hand-edited DB row that landed the same payload in the stored serialized blob — would propagate header injection through the Flask `after_request` merge. Werkzeug rejects CRLF at response-finalize on the currently-pinned version, but that pin is not a contractual security invariant; both guards remove the dependency on the downstream stack. The two call sites share a `_header_pair_is_safe` helper so the deserialize path cannot drift from parse.

### Added

-   `./ps-empire update` now applies pending Alembic migrations as part of the upgrade flow, so users no longer have to start the server, watch it crash on a schema mismatch, and chase the recovery advice manually. The new step (a) reads the current and head revisions, (b) prompts to back up the database and migrate (skip with `n`, or auto-confirm with `-y`), (c) backs up via the existing `backup_db()` helper, then (d) stamps the Alembic baseline if the database is untracked and applies pending migrations. Idempotent — a no-op when the schema is already at head. Backup failure (e.g. `mysqldump` not installed) warns but still proceeds with the migration, since refusing to migrate would leave the server unbootable. Migration failure surfaces the backup path so the operator can restore.
-   Smoothed the bcrypt → PBKDF2 upgrade path introduced in PR #1236. (1) On startup, if the configured default user (`database.defaults.username`) still has a bcrypt-shaped password hash, it is auto-reset to PBKDF2 of `database.defaults.password` and a WARNING is logged naming the auto-reset and the recovery procedure. Non-default users with bcrypt hashes are **not** touched and must be reset manually — keeps the auto-recovery from being usable as a privilege-escalation vector via DB tampering. (2) `verify_password()` now detects bcrypt-shaped hashes (`$2a$`/`$2b$`/`$2x$`/`$2y$`) and logs a targeted ERROR pointing at this CHANGELOG entry, instead of falling through to the PBKDF2 parser and surfacing an opaque `too many values to unpack` trace. **Manual recovery for non-default users:** as an admin, `POST /api/v2/users/{id}/password` with the new password (or reset via the Starkiller UI); the next login will store a PBKDF2 hash.
-   Added `./ps-empire update` subcommand that moves the Empire checkout to the latest release tag (via `setup/checkout-latest-tag.sh`, matching the documented install path), overwrites `~/.config/empire/config.yaml` from the shipped template, then fast-forwards the Starkiller and plugin-registry clones and refreshes the cached Empire-Compiler binary if the pinned release tag changed. Skips the source step on development branches (`sponsors-main`, `7.0-dev`, etc.) so users tracking a branch can manage upstream pulls themselves. Supports `-y` to auto-confirm cache-migration prompts. **BREAKING:** the base `config.yaml` is overwritten on every update — move any local customizations to `~/.config/empire/config.user.yaml` (the layered loader has merged `config.user.yaml` over the base since 7.0; see the *User Config Overrides* section of `docs/quickstart/server.md`).
-   Added downloadable chatlog report. The `basic_reporting` plugin now accepts `report=chat` (and includes it in `report=all`), exporting `chatlog.csv` with columns `Timestamp,Username,Message`. Operator chat messages are persisted to a new `chat_messages` table (alembic migration `0003`); previously they lived only in an in-memory list and were lost on every server restart. Also fixes an off-by-N bug in the `chat/history` socket handler that returned the first 20 messages instead of the most recent 20 once history exceeded 20 entries.
...
-   Parser support for the Cobalt Strike `http-config` top-level block in malleable profiles (`trust_x_forwarded_for`, `block_useragents`, `header "Name" "Value";`, `set headers "A, B, C";`). The malleable HTTP listener wires three of the four knobs at boot: (1) `trust_x_forwarded_for` (default **False** — defaulting to True is a spoofing vector without a trusted upstream proxy) derives the real client IP from the first entry of `X-Forwarded-For`, validates it as IPv4/IPv6 via `ipaddress.ip_address()`, and falls back to `request.remote_addr` on malformed input; (2) `block_useragents` accepts fnmatch globs (NOT regex — operator-pasted public CS profiles cannot trigger ReDoS), lowercases and dedups at parse time, caps UA bytes at 4 KiB before matching, and returns the same IIS 7.5 404 page used by the `host_stage` gate so the fingerprint is uniform; (3) `header "Name" "Value";` directives merge into every response (including the UA-block 404) via a Flask `after_request` hook using `setdefault()` so per-request transaction headers take precedence. The `set headers "A, B, C";` ordering hint is stored verbatim but the listener does not yet enforce ordering at the Flask layer. Captured at listener start — profile reload requires listener restart.
-   Parser support for the Cobalt Strike `https-certificate` top-level block in malleable profiles (`CN`, `O`, `OU`, `C`, `L`, `ST`, `validity`, `keystore`, `password`). **Parse-only** — Empire still loads the TLS cert from `CertPath` (built once by `setup/cert.sh`) and does not yet drive these fields into the underlying X.509. The listener emits a startup WARNING when an HTTPS listener has a profile that declares this block (including empty `https-certificate { }`), reporting the effective load path so operators know the profile values are not being honored. Runtime cert generation is deferred to a follow-up.
...
-   Updated default Starkiller ref from `sponsors-main` to `4.0-dev`. Existing installs with the old ref on disk will be prompted to migrate on the next `./ps-empire update` (or run `./ps-empire update -y` to adopt automatically).
...
-   **BREAKING:** Listeners and stagers now raise `ListenerValidationException` / `StagerGenerationException` for errors instead of returning error strings or `(False, "msg")` tuples. Plugin and module execution paths no longer return tuples; callers receive results directly with errors raised as exceptions.
...
-   Demoted the malleable profile "accepting `set <key>` but Empire does not act on it yet" log from INFO to DEBUG. Shipped Cobalt Strike profiles routinely declare directives Empire does not honor (the accepted-but-ignored allow-list spans 60+ keys across `dns-beacon`, `stage`, `process-inject`, `post-ex`, and SMB/TCP/SSH transport blocks); since Empire loads multiple profiles at startup, INFO flooded operator consoles at the default log level. Genuinely-unknown set directives (typos or new CS keys not yet allow-listed) still emit a WARNING.
...
-   Removed a byte-identical duplicate definition of `Profile._apply_set_directive` in `empire/server/common/malleable/profile.py` (46 lines, character-for-character matching the first definition). The duplicate was a merge-resolution artifact; Python used the second definition by virtue of class-body execution order, but the first was dead code.
...
-   Fixed `Profile._deserialize` and `HttpsCertificate._deserialize` using raw `bool(data.get(...))` for `host_stage`, `_was_declared`, and `trust_x_forwarded_for`, which silently mapped a stringified `"false"` back to `True` via `bool("false") is True`. The `host_stage` flip was security-relevant — a hand-edited DB row, an older serialized blob, or any future code path that stringified the profile would silently re-enable the stager URI on an operator-disabled listener. All three call sites now use a centralized `_coerce_bool` helper that mirrors the parse-side vocabulary (`true`, `1`, `yes`, `on` → True; everything else → False). The matching `_coerce_optional_int` helper applied to `https-certificate validity` log+drops malformed values instead of raising up to `MalleableError` and killing listener startup.
-   Fixed the malleable HTTP listener raising `AttributeError: 'NoneType' object has no attribute 'extract_client'` and surfacing a Flask 500 when a request URI matched none of the configured `http-get`, `http-post`, or `http-stager` URIs. The dispatcher previously logged the "unknown uri" WARNING but then fell through to `implementation.extract_client(...)` with `implementation=None`. Now returns the shared IIS 7.5 404 page that the `host_stage` gate and `block_useragents` fast-path also use, preserving the uniform-fingerprint property — any non-blocked scanner probing arbitrary paths could otherwise have detected the listener via the distinctive 500 response. Bug pre-dates this branch (regression from PR adding the URI dispatch logic in July 2024) but is fixed here as a sibling concern to the http-config / https-certificate work.
...
-   Preserved subprocess diagnostics across stager compile failures. `GoCompiler.compile_stager` and `DotnetCompiler.compile_task`/`compile_stager` now include `returncode` in the raised `ModuleExecutionException` and fall back through `stderr → stdout → "<no output — likely killed by signal>"` when stderr is empty (the fingerprint of OOM/cgroup/AV kills, which previously surfaced as `Go build failed: ` with no detail). The full streams are also written to `log.error` before the raise. Separately, `compile_stager`'s env merge order flipped to `{**os.environ, **env}` so explicit `goos`/`goarch` args win over an operator-set `GOOS`/`GOARCH` in the shell. `stager_service.generate_stager` now catches `ModuleExecutionException` from `template_instance.generate()` and returns `(None, str(e))` so the API surfaces a 400 with the message instead of a 500 stack trace; logged with `exc_info=True` for empire-side debugging.
-   Removed the instance-level `threading.Lock` from `AgentCommunicationService` that was serializing all concurrent agent file writes across all sessions. Each agent writes to its own `downloads/<session_id>/` subdirectory and the per-agent request/response model already guarantees same-session chunks arrive sequentially, so the lock added no safety — only a single-threaded bottleneck through which every parallel download had to queue. Multi-agent download throughput now scales with the worker pool. The change also fixes `save_file` leaving a zero-byte file on disk when python/go agent decompression failed (decompression now runs before `open()`, and the write is wrapped in a `with` context manager), and guards the download-progress-percent calculation against `ZeroDivisionError`/`ValueError`/`OSError` from a malformed agent-supplied `total_filesize`.
