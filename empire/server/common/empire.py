...
class MainMenu(object):
    ...
    def bottom_toolbar(self):
        return HTML(
            ...
            + " Agent(s) | "
            + str(len(self.listenersv2.get_active_listeners()))
            + " Listener(s) | "
            + str(len(self.pluginsv2.get_all()))
            + " Plugin(s)"
        )
    ...
    def run_report_query(self):
        with SessionLocal.begin() as db:
            reporting_sub_query = (
                db.query(
                    models.Reporting,
                    self.substring(db, models.Reporting.name, "/").label("agent_name"),
                )
                .filter(
                    and_(
            ...
            return (
                ...
                .join(
                    models.User, models.User.id == models.Tasking.user_id, isouter=True
                )
                .join(
                    models.Agent,
