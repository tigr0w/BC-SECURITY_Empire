...
class Module(object):
    ...
    def generate(
        ...
        agent_code = main_menu.listenertemplatesv2.new_instance(
            active_listener["moduleName"]
        ).generate_agent(active_listener.options, language=language, version=version)
        ...
        if language.lower() == "powershell":
            ...
            stager_code = main_menu.listenertemplatesv2.new_instance(
                active_listener["moduleName"]
            ).generate_stager(active_listener.options, language=language, encrypt=False)
