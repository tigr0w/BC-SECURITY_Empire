...
class Listener(object):
    ...
    def generate_launcher(
        ...
        if listenerName and (listenerName in self.mainMenu.listeners.activeListeners):
            ...
            moduleSourcePath = (
                "%s/data/module_source/code_execution/Invoke-Shellcode.ps1"
    ...
    def generate_comms(self, language=None):
        ...
        if language:
            if language.lower() == "powershell":
                ...
                print(
                    helpers.color(
                        "[!] listeners/meterpreter generate_comms(): invalid language specification, only 'powershell' and 'python' are current supported for this module."
                    )
                )
        else:
            print(
                helpers.color(
                    "[!] listeners/meterpreter generate_comms(): no language specified!"
                )
            )
