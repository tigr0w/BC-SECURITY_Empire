...
from empire.server.v2.api.agent.task_dto import TaskOrderOptions, ModulePostRequest
...
class AgentTaskService(object):
    ...
    def get_tasks(db: Session,
        ...
        query = db \
            .query(models.Tasking, func.count(models.Tasking.id).over().label('total'))
        ...
        query_options = [joinedload(models.Tasking.user), joinedload(models.Tasking.agent).joinedload(models.Agent.host)]
    ...
    @staticmethod
    def get_task_for_agent(db: Session,
                           agent_id: str,
                           uid: int):
        return db\
            .query(models.Tasking)\
            .filter(and_(models.Tasking.agent_id == agent_id,
                         models.Tasking.id == uid)) \
            .first()

    def create_task_shell(self, db: Session, agent: models.Agent, command: str, user_id: int):
        return self.add_task(db, agent, "TASK_SHELL", command, user_id=user_id)

    def create_task_upload(self, db: Session, agent: models.Agent, file_data: str, directory: str, user_id):
        data = f'{directory}|{file_data}'
        return self.add_task(db, agent, "TASK_UPLOAD", data, user_id=user_id)

    def create_task_download(self, db: Session, agent: models.Agent, path_to_file: str, user_id: int):
        return self.add_task(db, agent, 'TASK_DOWNLOAD', path_to_file, user_id=user_id)

    def create_task_script_import(self, db: Session, agent: models.Agent, file_data: str, user_id: int):
        ...
        return self.add_task(db, agent, 'TASK_SCRIPT_IMPORT', file_data, user_id=user_id)

    def create_task_script_command(self, db: Session, agent: models.Agent, command: str, user_id: int):
    ...
    def create_task_update_comms(self, db: Session, agent: models.Agent, new_listener_id: int, user_id: int):
        ...
        if listener.module in ['meterpreter', 'http_mapi']:
            return None, f'Listener template {listener.module} not eligible for updating comms'

        # todo separate methods for getting db_listeners and instances?
        new_comms = self.listener_service.get_active_listeners()[listener.id].generate_comms(listener.options, agent.language)

        self.add_task(db, agent, "TASK_UPDATE_LISTENERNAME", listener.name, user_id=user_id)
        return self.add_task(db, agent, "TASK_SWITCH_LISTENER", new_comms, user_id=user_id)

    def create_task_update_sleep(self, db: Session, agent: models.Agent, delay: int, jitter: float, user_id: int):
        agent.delay = delay
        agent.jitter = jitter
        if agent.language == 'powershell':
            return self.add_task(db, agent, 'TASK_SHELL', f'Set-Delay {str(delay)} {str(jitter)}', user_id=user_id)
        elif agent.language == 'python':
            return self.add_task(db, agent, "TASK_CMD_WAIT",
                                 f"global delay; global jitter; delay={delay}; jitter={jitter}; print('delay/jitter set to {delay}/{jitter}')", user_id=user_id)
        elif agent.language == 'csharp':
            return self.add_task(db, agent, 'TASK_SHELL', f'Set-Delay {str(delay)} {str(jitter)}', user_id=user_id)
        else:
            return None, 'Unsupported language.'

    def create_task_update_kill_date(self, db: Session, agent: models.Agent, kill_date: str, user_id: int):
        # todo handle different languages
        agent.kill_date = kill_date
        return self.add_task(db, agent, "TASK_SHELL", f"Set-KillDate {kill_date}", user_id=user_id)

    def create_task_update_working_hours(self, db: Session, agent: models.Agent, working_hours: str, user_id: int):
        # todo handle different languages.
        agent.working_hours = working_hours
        return self.add_task(db, agent, "TASK_SHELL", f"Set-WorkingHours {working_hours}", user_id=user_id)

    def create_task_module(self, db: Session, agent: models.Agent, module_req: ModulePostRequest, user_id: int):
        module_req.options['Agent'] = agent.session_id
        resp, err = self.module_service.execute_module(module_req.module_slug, module_req.options, module_req.ignore_language_version_check, module_req.ignore_admin_check)

        if err:
            return None, err

        return self.add_task(db, agent,
                             task_name=resp['command'],
                             task_input=resp['data'],
                             module_name=module_req.module_slug,
                             user_id=user_id)

    def create_task_directory_list(self, db: Session, agent: models.Agent, path: str, user_id: int):
        return self.add_task(db, agent, "TASK_DIR_LIST", path, user_id=user_id)

    def create_task_proxy_list(self, db: Session, agent: models.Agent, body: Dict, user_id: int):
        agent.proxy = body
        return self.add_task(db, agent, "TASK_SET_PROXY", json.dumps(body), user_id=user_id)

    @staticmethod
    def add_task(
        db: Session, agent: models.Agent, task_name, task_input="", module_name: str = None, user_id: int = 0
        ...
        pk = db.query(func.max(models.Tasking.id)).filter(models.Tasking.agent_id == agent.session_id).first()[0]
        ...
        signal = json.dumps(
            {"print": True, "message": message, "task_name": task_name, "task_id": pk, "task": task_input, "event_type": "task"}
        )
