...
from empire.server.v2.api.plugin.plugin_dto import domain_to_dto_plugin, PluginExecutePostRequest
...
async def read_plugins():
    plugins = list(map(lambda x: domain_to_dto_plugin(x[1], x[0]),
                       plugin_service.get_all().items()))

    return {'records': plugins}


@router.get("/{uid}", dependencies=[Depends(get_current_active_user)])
async def read_plugin(uid: str,
...
async def execute_module(uid: str,
    ...
    for option, values in plugin.options.items():
        ...
        if values['Strict'] and values['Value'] not in values['SuggestedValues']:
            raise HTTPException(400, f'{option} must be set to one of suggested values.')
