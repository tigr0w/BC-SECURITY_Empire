...
from fastapi import HTTPException, Depends, UploadFile, Form, File, Query
...
from empire.server.v2.api.agent.task_dto import TaskOrderOptions, domain_to_dto_task, ShellPostRequest, \
    ModulePostRequest, DownloadPostRequest, CommsPostRequest, Task, Tasks, KillDatePostRequest, WorkingHoursPostRequest, \
    SysinfoPostRequest, ScriptCommandPostRequest, SleepPostRequest, ExitPostRequest, DirectoryListPostRequest, \
    ProxyListPostRequest
from empire.server.v2.api.jwt_auth import get_current_user, get_current_active_user
...
async def get_task(uid: int,
    ...
    raise HTTPException(404, f'Task not found for agent {db_agent.session_id} and task id {uid}')


@router.get("/tasks", response_model=Tasks)
async def get_tasks_all_agents(limit: int = -1,
                               page: int = 1,
                               ...
                               db: Session = Depends(get_db)):
    tasks, total = agent_task_service.get_tasks(db,
                                 agents=agents,
                                                users=users,
                                 limit=limit,
                                 offset=(page-1)*limit,
    ...
    tasks_converted = list(map(lambda x: domain_to_dto_task(x, include_full_input, include_original_output, include_output),
                     tasks))

    return Tasks(records=tasks_converted, page=page, total_pages=math.ceil(total/limit), limit=limit, total=total)


@router.get("/{agent_id}/tasks", response_model=Tasks)
async def read_tasks(limit: int = -1,
                     page: int = 1,
                     include_full_input: bool = False,
                     include_original_output: bool = False,
                     include_output: bool = True,
...
async def create_task_shell(shell_request: ShellPostRequest,
                            db_agent: models.Agent = Depends(get_agent),
                            db: Session = Depends(get_db),
                            current_user: models.User = Depends(get_current_user)):
    resp, err = agent_task_service.create_task_shell(db, db_agent, shell_request.command, current_user.id)
...
async def create_task_module(module_request: ModulePostRequest,
                             db_agent: models.Agent = Depends(get_agent),
                             db: Session = Depends(get_db),
                             current_user: models.User = Depends(get_current_user)):
    resp, err = agent_task_service.create_task_module(db, db_agent, module_request, current_user.id)
...
async def create_task_upload(file: UploadFile = File(...),
    ...
    if len(raw_data) > 1048576:
        raise HTTPException(status_code=400, detail='file size too large. Maximum file size of 1MB')

    resp, err = agent_task_service.create_task_upload(db, db_agent, file_data, path_to_file, current_user.id)
...
async def create_task_download(download_request: DownloadPostRequest,
                               db_agent: models.Agent = Depends(get_agent),
                               db: Session = Depends(get_db),
                               current_user: models.User = Depends(get_current_user)):
    resp, err = agent_task_service.create_task_download(db, db_agent, download_request.path_to_file, current_user.id)
...
async def create_task_script_import(file: UploadFile = File(...),
                                    db_agent: models.Agent = Depends(get_agent),
                                    db: Session = Depends(get_db),
                                    current_user: models.User = Depends(get_current_user)):
    file_data = await file.read()
    file_data = file_data.decode('utf-8')
    resp, err = agent_task_service.create_task_script_import(db, db_agent, file_data, current_user.id)
...
async def create_task_script_command(script_command_request: ScriptCommandPostRequest,
                                     db_agent: models.Agent = Depends(get_agent),
                                     db: Session = Depends(get_db),
                                     current_user: models.User = Depends(get_current_user)):
    """
    ...
    :return:
    """
    resp, err = agent_task_service.create_task_script_command(db, db_agent, script_command_request.command,
                                                              current_user.id)
...
async def create_task_sysinfo(sysinfo_request: SysinfoPostRequest,  # todo empty atm
                              db_agent: models.Agent = Depends(get_agent),
                              db: Session = Depends(get_db),
                              current_user: models.User = Depends(get_current_user)):
...
async def create_task_update_comms(comms_request: CommsPostRequest,
                                   db_agent: models.Agent = Depends(get_agent),
                                   db: Session = Depends(get_db),
                                   current_user: models.User = Depends(get_current_user)):
    resp, err = agent_task_service.create_task_update_comms(db, db_agent, comms_request.new_listener_id,
                                                            current_user.id)
...
async def create_task_update_sleep(sleep_request: SleepPostRequest,
                                   db_agent: models.Agent = Depends(get_agent),
                                   db: Session = Depends(get_db),
                                   current_user: models.User = Depends(get_current_user)):
    resp, err = agent_task_service.create_task_update_sleep(db, db_agent, sleep_request.delay,
                                                                sleep_request.jitter,
                                                                current_user.id)
...
async def create_task_update_kill_date(kill_date_request: KillDatePostRequest,
                                       db_agent: models.Agent = Depends(get_agent),
                                       db: Session = Depends(get_db),
                                       current_user: models.User = Depends(get_current_user)):
    resp, err = agent_task_service.create_task_update_kill_date(db, db_agent, kill_date_request.kill_date,
                                                                current_user.id)
...
async def create_task_update_working_hours(working_hours_request: WorkingHoursPostRequest,
                                           db_agent: models.Agent = Depends(get_agent),
                                           db: Session = Depends(get_db),
                                           current_user: models.User = Depends(get_current_user)):
    resp, err = agent_task_service.create_task_update_working_hours(db, db_agent, working_hours_request.working_hours,
                                                                    current_user.id)
...
async def create_task_update_directory_list(directory_list_request: DirectoryListPostRequest,
                                           db_agent: models.Agent = Depends(get_agent),
                                           db: Session = Depends(get_db),
                                           current_user: models.User = Depends(get_current_user)):
    resp, err = agent_task_service.create_task_directory_list(db, db_agent, directory_list_request.path,
                                                                    current_user.id)
...
async def create_task_update_proxy_list(proxy_list_request: ProxyListPostRequest,
                                        db_agent: models.Agent = Depends(get_agent),
                                        db: Session = Depends(get_db),
                                        current_user: models.User = Depends(get_current_user)):
    ...
    resp, err = agent_task_service.create_task_proxy_list(db, db_agent, proxy_list_dict, current_user.id)
...
async def create_task_exit(exit_request: ExitPostRequest,
                                           db_agent: models.Agent = Depends(get_agent),
                                           db: Session = Depends(get_db),
                                           current_user: models.User = Depends(get_current_user)):
...
async def delete_task(uid: int,
                      db: Session = Depends(get_db),
                      db_task: models.Tasking = Depends(get_task)):
    if db_task.status != TaskingStatus.queued:
        raise HTTPException(status_code=400, detail='Task must be in a queued state to be deleted')
