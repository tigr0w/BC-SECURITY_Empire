...
from empire.server.v2.api.agent.agent_dto import Agent, domain_to_dto_agent, AgentUpdateRequest, Agents
...
async def read_agent(uid: str,
                     db_agent: models.Agent = Depends(get_agent)):
    return domain_to_dto_agent(db_agent)


@router.get("/", response_model=Agents)
async def read_agents(db: Session = Depends(get_db),
                      include_archived: bool = False,
                      include_stale: bool = True):
    agents = list(map(lambda x: domain_to_dto_agent(x), agent_service.get_all(db, include_archived, include_stale)))

    return {'records': agents}


@router.put("/{uid}", response_model=Agent)
async def update_agent(uid: str,
