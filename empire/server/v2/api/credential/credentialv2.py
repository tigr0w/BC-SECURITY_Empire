...
from empire.server.v2.api.credential.credential_dto import Credential, domain_to_dto_credential, \
    CredentialUpdateRequest, CredentialPostRequest, Credentials
...
@router.get("/{uid}", response_model=Credential, dependencies=[Depends(get_current_active_user)])
async def read_credential(uid: int,
                          db_credential: models.Credential = Depends(get_credential)):
    return domain_to_dto_credential(db_credential)


@router.get("/", response_model=Credentials, dependencies=[Depends(get_current_active_user)])
async def read_credentials(db: Session = Depends(get_db)):
    credentials = list(map(lambda x: domain_to_dto_credential(x), credential_service.get_all(db)))

    return {'records': credentials}


@router.post('/', status_code=201, response_model=Credential, dependencies=[Depends(get_current_active_user)])
async def create_credential(credential_req: CredentialPostRequest,
...
@router.put("/{uid}", response_model=Credential, dependencies=[Depends(get_current_active_user)])
async def update_credential(uid: int,
