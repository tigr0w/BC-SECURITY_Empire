...
from empire.server.v2.api.module.module_dto import domain_to_dto_module, Module, Modules, ModuleUpdateRequest, \
...
async def read_modules():
    ...
    modules = list(map(lambda x: domain_to_dto_module(x[1], x[0]),
                       module_service.get_all().items()))
...
@router.get("/{uid}", response_model=Module, dependencies=[Depends(get_current_active_user)])
async def read_module(uid: str,
                      module: PydanticModule = Depends(get_module)):
    return domain_to_dto_module(module, uid)


@router.put("/{uid}", response_model=Module, dependencies=[Depends(get_current_active_user)])
async def update_module(uid: str,
...
@router.put("/bulk/enable", status_code=204, dependencies=[Depends(get_current_active_user)])
async def update_bulk_enable(module_req: ModuleBulkUpdateRequest,
                             db: Session = Depends(get_db)):
