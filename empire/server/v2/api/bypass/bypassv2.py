...
from empire.server.v2.api.bypass.bypass_dto import Bypass, domain_to_dto_bypass, BypassUpdateRequest, BypassPostRequest, \
    Bypasses
...
async def create_bypass(bypass_req: BypassPostRequest,
                        db: Session = Depends(get_db)):
...
async def update_bypass(uid: int,
...
async def delete_bypass(uid: str,
                        db: Session = Depends(get_db),
                        db_bypass: models.Bypass = Depends(get_bypass)):
