...
PROXY_IDS = {}
for name, ID in list(PROXY_NAME.items()):
    PROXY_IDS[ID] = name
...
def start_restful_api(
    ...
    def generate_stager():
        ...
        if (
            not request.json
            or not "StagerName" in request.json
            or not "Listener" in request.json
        ):
            abort(400)
        ...
        if stager_name not in main.stagers.stagers:
            return make_response(
                jsonify({"error": "stager name %s not found" % (stager_name)}), 404
            )
            ...
            if option != "StagerName":
                if option not in stager.options:
                    return make_response(
                        jsonify(
                            {
                                "error": "Invalid option %s, check capitalization."
                                % (option)
                            }
                        ),
                        400,
        ...
        for option, values in stager.options.items():
            if values["Required"] and (
                (not values["Value"]) or (values["Value"] == "")
            ):
                return make_response(
                    jsonify({"error": "required stager options missing"}), 400
                )
            if values["Strict"] and values["Value"] not in values["SuggestedValues"]:
                return make_response(
                    jsonify(
                        {
                            "error": f"{option} must be set to one of the suggested values."
                        }
                    )
                )
    ...
    def get_module_name(module_name):
        ...
        if module_name not in main.modules.modules:
            return make_response(
                jsonify({"error": "module name %s not found" % (module_name)}), 404
            )
    ...
    def execute_module(module_name):
        ...
        if not module:
            return make_response(
                jsonify({"error": f"module name {module_name} not found"}), 404
            )

        result, err = main.modules.execute_module(
            module, params=request.json, user_id=g.user["id"]
        )
    ...
    def get_listeners():
        ...
        for active_listener in active_listeners_raw:
            listeners.append(
                {
                    "ID": active_listener.id,
                    "name": active_listener.name,
                    "module": active_listener.module,
    ...
    def validate_listeners(listener_type):
        ...
        if listener_type.lower() not in main.listeners.loadedListeners:
            return make_response(
                jsonify({"error": f"listener type {listener_type} not found"}), 404
            )
        ...
        for option, values in request.json.items():
            ...
            return_options = main.listeners.set_listener_option(
                listener_type, option, values
            )
            if not return_options:
                return make_response(
                    jsonify(
                        {
                            "error": "error setting listener value %s with option %s"
                            % (option, values)
                        }
                    ),
                    400,
        ...
        if validation == True:
            ...
            return jsonify(
                {"error": "failed to validate listener %s options" % listener_name}
            )
        else:
    ...
    def get_listener_name(listener_name):
        ...
        if not active_listener:
            return make_response(
                jsonify({"error": "listener name %s not found" % listener_name}), 404
            )
    ...
    def kill_listener(listener_name):
        ...
        if listener_name.lower() == "all":
            ...
            if listener_name != "" and main.listeners.is_listener_valid(listener_name):
                ...
                return make_response(
                    jsonify({"error": "listener name %s not found" % listener_name}),
                    404,
                )
    ...
    def disable_listener(listener_name):
        ...
        if listener_name != "" and main.listeners.is_listener_valid(listener_name):
            ...
            return make_response(
                jsonify(
                    {
                        "error": "listener name %s not found or already disabled"
                        % listener_name
                    }
                ),
                404,
    ...
    def enable_listener(listener_name):
        ...
        if (
            listener_name != ""
            and listener_name in main.listeners.get_inactive_listeners()
        ):
            ...
            return make_response(
                jsonify(
                    {
                        "error": "listener name %s not found or already enabled"
                        % listener_name
                    }
                ),
                404,
    ...
    def edit_listener(listener_name):
        ...
        if main.listeners.is_listener_valid(listener_name):
            return make_response(
                jsonify({"error": "Provided listener should be disabled"}), 400
            )
        ...
        if listener_name in main.listeners.get_inactive_listeners():
            ...
            success = main.listeners.update_listener_options(
                listener_name, option_name, option_value
            )
            if success:
                return jsonify({"success": True})
            else:
            ...
            return make_response(
                jsonify(
                    {
                        "error": "listener name %s not found or not inactive"
                        % listener_name
                    }
                ),
                404,
    ...
    def get_listener_options(listener_type):
        ...
        if listener_type.lower() not in main.listeners.loadedListeners:
            return make_response(
                jsonify({"error": "listener type %s not found" % listener_type}), 404
            )
    ...
    def start_listener(listener_type):
        ...
        if dupe_check:
            return make_response(
                jsonify(
                    {"error": f"listener with name {listener_name} already exists"}
                ),
                400,
            )
        ...
        for option, values in request.json.items():
            ...
            returnVal = main.listeners.set_listener_option(
                listener_type, option, values
            )
            if not returnVal:
                return make_response(
        ...
        if listenerID:
            return jsonify(
                {"success": "Listener %s successfully started" % listener_name}
            )
        else:
    ...
    def get_host_process(agent_name):
        ...
        agent = (
            Session()
            .query(models.Agent)
            .filter(models.Agent.session_id == agent_name)
            .first()
        )
        processes = []
        if agent:
            processes_raw: List[models.HostProcess] = (
                Session()
                .query(models.HostProcess)
                .filter(models.HostProcess.host_id == agent.host_id)
                .all()
            )
    ...
    def scrape_agent_directory(agent_name):
        ...
        task_id = main.agents.add_agent_task_db(
            agent_name, "TASK_DIR_LIST", directory, g.user["id"]
        )
    ...
    def get_agent_directory(agent_name):
        ...
        found = (
            ...
            .filter(
                and_(
                    ...
                    models.AgentFile.path == directory,
                )
            )
            .first()
        )
        ...
        results = (
            ...
            .join(agent_file_alias, models.AgentFile.parent_id == agent_file_alias.id)
            .filter(
                and_(
                    models.AgentFile.session_id == agent_name,
                    agent_file_alias.path == directory,
                )
            )
            .all()
        )
            ...
            response.append(
                {
                    ...
                    "parent_parent": result.parent_parent,
                }
            )
    ...
    def get_agent_tasks(agent_name):
        ...
        if not request.args.get("num_results"):
            return make_response(
                jsonify({"error": "number of results to return not provided"}), 404
            )
    ...
    def task_agent_download(agent_name):
        ...
        task_id = main.agents.add_agent_task_db(
            agent.session_id, "TASK_DOWNLOAD", file_name, uid=g.user["id"]
        )
    ...
    def task_agent_upload(agent_name):
        ...
        if len(raw_bytes) > 1048576:
            return make_response(
                jsonify({"error": "file size too large, upload limit: <1MB."}), 404
            )

        msg = "Tasked agent to upload %s : %s" % (
            file_name,
        ...
        task_id = main.agents.add_agent_task_db(
            agent.session_id, "TASK_UPLOAD", data, uid=g.user["id"]
        )
    ...
    def task_agent_shell(agent_name):
        ...
        if command == "sysinfo":
            ...
            main.agents.save_agent_log(agent.session_id, msg)
            task_id = main.agents.add_agent_task_db(
                agent.session_id, "TASK_SHELL", command, uid=g.user["id"]
            )
    ...
    def set_agent_sleep(agent_name):
        ...
        if (
            not request.json
            or "delay" not in request.json
            or "jitter" not in request.json
        ):
            return make_response(
                jsonify({"error": "Jitter and sleep interval are not provided"}), 400
            )
        ...
        if agent_delay >= 0:
            ...
            return make_response(
                jsonify({"error": "Delay must be a positive integer"}), 400
            )
        ...
        if agent_jitter >= 0 and agent_jitter <= 1:
            ...
            return make_response(
                jsonify({"error": "Jitter must be between 0.0 and 1.0"}), 400
            )
        ...
        if agent.language == "powershell":
            ...
            task_id = main.agents.add_agent_task_db(
                ...
                % (agent_delay, agent_jitter, agent_delay, agent_jitter),
            )
        ...
        # dispatch this event
        msg = "[*] Tasked agent to sleep delay/jitter {}/{}".format(
            agent_delay, agent_jitter
        )
        main.agents.save_agent_log(agent.session_id, msg)
    ...
    def task_agent_script_command(agent_name):
        ...
        main.agents.save_agent_log(agent.session_id, msg)
        task_id = main.agents.add_agent_task_db(
            agent.session_id, "TASK_SCRIPT_COMMAND", command, uid=g.user["id"]
        )
    ...
    def agent_update_comms(agent_name):
        ...
        if not "listener" in request.json:
            return make_response(
                jsonify({"error": 'JSON body must include key "listener"'}), 400
            )
    ...
    def get_proxy_info(agent_name):
        ...
        proxy_info = {
            ...
            "options": {
                "Address": {
                    "Description": "Address for the proxy.",
                    "Required": True,
                    "Value": "",
                    "SuggestedValues": "",
                    "Strict": "",
    ...
    def agent_update_proxy(agent_name):
        ...
        main.agents.add_agent_task_db(
            agent_name, "TASK_SET_PROXY", json.dumps(proxy_list)
        )
    ...
    def agent_kill_date(agent_name):
        ...
        if not "kill_date" in request.json:
            return make_response(
                jsonify({"error": 'JSON body must include key "kill_date"'}), 400
            )
        ...
        try:
            ...
            # task the agent
            main.agents.add_agent_task_db(
                agent_name, "TASK_SHELL", "Set-KillDate " + str(kill_date)
            )
    ...
    def agent_working_hours(agent_name):
        ...
        if not "working_hours" in request.json:
            return make_response(
                jsonify({"error": 'JSON body must include key "working_hours"'}), 400
            )
        ...
        try:
            ...
            # task the agent
            main.agents.add_agent_task_db(
                agent_name, "TASK_SHELL", "Set-WorkingHours " + str(working_hours)
            )
    ...
    def task_agent_rename(agent_name):
        ...
        try:
            ...
            return make_response(
                jsonify(
                    {"error": "error in renaming %s to %s" % (agent_name, new_name)}
                ),
                400,
            )
    ...
    def update_agent_notes(agent_name):
        ...
        if not "notes" in request.json:
            return make_response(
                jsonify({"error": 'JSON body must include key "notes"'}), 400
            )

        agent = main.agents.get_agent_from_name_or_session_id(agent_name)
        if not agent:
            return make_response(
                jsonify({"error": f"Agent not found with name {agent_name}"})
            )
    ...
    def get_cred(uid):
        ...
        credential = (
            ...
            .filter(models.Credential.id == uid)
            .first()
        )
    ...
    def add_creds():
        ...
        if not (cred["credtype"] == "hash" or cred["credtype"] == "plaintext"):
            return make_response(
                jsonify(
                    {
                        "error": 'invalid credential type in credtype, must be "hash" or "plaintext"'
                    }
                ),
                400,
            )
        ...
        return make_response(
            jsonify(
                {
                    "error": f"Error writing credential. Check you aren't writing a duplicate."
                }
            ),
            400,
        )
    ...
    def edit_cred(uid):
        ...
        # ensure the type is either "hash" or "plaintext"
        if not (
            request.json["credtype"] == "hash"
            or request.json["credtype"] == "plaintext"
        ):
            return make_response(
                jsonify(
                    {"error": 'invalid credential type, must be "hash" or "plaintext"'}
                ),
                400,
            )
    ...
    def get_reporting():
        ...
        for reporting_event in reporting_raw:
            reporting_events.append(
                {
                    "timestamp": reporting_event.timestamp,
                    "event_type": reporting_event.event_type,
                    "username": reporting_event.username,
                    "agent_name": reporting_event.agent_name,
                    "host_name": reporting_event.hostname,
                    "taskID": reporting_event.taskID,
                    "task": reporting_event.task,
    ...
    def get_reporting_agent(reporting_agent):
        ...
        session_id = (
            ...
            .filter(models.Agent.name == reporting_agent)
            .scalar()
        )
        if not session_id:
        ...
        reporting_raw = (
            Session()
            .query(models.Reporting)
            .filter(models.Reporting.name.contains(session_id))
            .all()
        )
        reporting_events = []
            ...
            reporting_events.append(
                {
                    ...
                    "agentname": reporting_event.name,
                    "event_type": reporting_event.event_type,
                    "message": json.loads(reporting_event.message),
                    "timestamp": reporting_event.timestamp,
                    "taskID": reporting_event.taskID,
                }
            )
    ...
    def get_reporting_type(event_type):
        ...
        reporting_raw = (
            Session()
            .query(models.Reporting)
            .filter(models.Reporting.event_type == event_type)
            .all()
        )
        reporting_events = []
    ...
    def get_reporting_msg(msg):
        ...
        reporting_raw = (
            Session()
            .query(models.Reporting)
            .filter(models.Reporting.message.contains(msg))
            .all()
        )
        reporting_events = []
    ...
    def get_malleable_profile(profile_name):
        ...
        return make_response(
            jsonify({"error": f"malleable profile {profile_name} not found"}), 404
        )
    ...
    def add_malleable_profile():
        ...
        if (
            ...
            or "category" not in request.json
        ...
        return make_response(
            jsonify({"error": f"malleable profile {profile_name} already exists"}), 400
        )
    ...
    def create_bypass():
        ...
        bypass = (
            Session().query(models.Bypass).filter(models.Bypass.name == name).first()
        )
        if not bypass:
    ...
    def get_users():
        ...
        for reporting_users in users_raw:
            data = {
                "ID": reporting_users.id,
                "username": reporting_users.username,
                "last_logon_time": reporting_users.last_logon_time,
                "enabled": reporting_users.enabled,
    ...
    def create_user():
        ...
        status = main.users.add_new_user(
    ...
    def get_plugin(plugin_name):
        ...
        if plugin_name not in empireMenu.loadedPlugins.keys():
            try:
                ...
                return make_response(
                    jsonify({"error": "plugin %s not found" % plugin_name}), 400
                )
        ...
        description = empireMenu.loadedPlugins[plugin_name].get_commands()[
            "description"
        ]
    ...
    def execute_plugin(plugin_name):
        ...
        if plugin_name not in empireMenu.loadedPlugins.keys():
            return make_response(
                jsonify({"error": "plugin %s not loaded" % plugin_name}), 404
            )
        ...
        for option, values in use_plugin.options.items():
            if values["Required"] and (
                ...
                return make_response(
                    jsonify({"error": "required module option missing"}), 400
                )
            if values["Strict"] and values["Value"] not in values["SuggestedValues"]:
                return make_response(
                    jsonify(
                        {"error": f"{option} must be set to one of suggested values."}
                    ),
                    400,
                )
    ...
    def server_startup_validator():
        ...
        if response:
            ...
            try:
                ...
                sio.connect(
                    f'wss://{args.restip}:{args.socketport}?token={response.json()["token"]}'
                )
