def test_get_profile_not_found(client, admin_auth_header):
    response = client.get('/api/v2beta/malleable-profiles/9999', headers=admin_auth_header)
...
def test_create_profile(client, admin_auth_header):
    response = client.post('/api/v2beta/malleable-profiles/',
...
def test_update_profile_not_found(client, admin_auth_header):
    response = client.put('/api/v2beta/malleable-profiles/9999',
                          headers=admin_auth_header,
                          json={
        'name': 'Test Profile',
        'category': 'cat',
...
def test_update_profile(client, admin_auth_header):
    response = client.put('/api/v2beta/malleable-profiles/1',
                          headers=admin_auth_header,
                          json={
        'data': 'x=1;'
    })
...
def test_delete_profile(client, admin_auth_header):
    response = client.delete('/api/v2beta/malleable-profiles/1', headers=admin_auth_header)
