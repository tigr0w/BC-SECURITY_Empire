...
def processes(db, hosts):
    ...
    if not processes:
        process1 = models.HostProcess(host_id=hosts[0].id,
                                      process_id='11',
        ...
        process2 = models.HostProcess(host_id=hosts[0].id,
                                      process_id='12',
...
def test_get_process_not_found(client, admin_auth_header, hosts):
    response = client.get(f'/api/v2beta/hosts/{hosts[0].id}/processes/8888', headers=admin_auth_header)

    assert response.status_code == 404
    assert response.json()['detail'] == f'Process not found for host id {hosts[0].id} and process id 8888'


def test_get_process(client, admin_auth_header, hosts, processes):
    response = client.get(f'/api/v2beta/hosts/{hosts[0].id}/processes/{processes[0].process_id}', headers=admin_auth_header)
...
def test_get_processes(client, admin_auth_header, hosts):
    response = client.get(f'/api/v2beta/hosts/{hosts[0].id}/processes/', headers=admin_auth_header)
...
def test_agent_join(client, admin_auth_header, hosts, agent):
    ...
    assert response.status_code == 200
    assert len(list(filter(lambda x: x['agent'] == agent.session_id, response.json()['records']))) == 1
