...
def test_create_bypass_name_conflict(client, admin_auth_header):
    response = client.post('/api/v2beta/bypasses/',
    ...
    assert response.json()['detail'] == 'Bypass with name mattifestation already exists.'
...
def test_update_bypass_not_found(client, admin_auth_header):
    response = client.put('/api/v2beta/bypasses/9999',
                          headers=admin_auth_header,
                          json={
                              'name': 'Test Bypass',
                              'code': 'x=0;'
...
def test_update_bypass_name_conflict(client, admin_auth_header):
    response = client.put('/api/v2beta/bypasses/5',
                          headers=admin_auth_header,
                          json={
                              'name': 'mattifestation',
                              'code': 'x=0;'
...
def test_update_bypass(client, admin_auth_header):
    response = client.put('/api/v2beta/bypasses/1',
                          headers=admin_auth_header,
                          json={
                              'name': 'Updated Bypass',
                              'code': 'x=1;'
