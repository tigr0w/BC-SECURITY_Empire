...
def plugin_task_1(main, session_local, models, plugin_id):
...
def test_get_tasks_for_plugin(client, admin_auth_header, plugin_id):
    response = client.get(
    ...
    assert (
        len(
            list(
                filter(
                    lambda x: x["plugin_id"] != plugin_id,
...
def test_get_task_for_plugin_not_found(client, admin_auth_header, plugin_id):
    response = client.get(
...
def test_get_task_for_plugin(client, admin_auth_header, plugin_id, plugin_task_1):
    response = client.get(
        f"/api/v2/plugins/{plugin_id}/tasks/{plugin_task_1}",
