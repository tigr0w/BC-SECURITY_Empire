...
def test_create_credential(client, admin_auth_header, base_credential):
    response = client.post('/api/v2beta/credentials/', headers=admin_auth_header, json=base_credential)
...
def test_create_credential_unique_constraint_failure(client, admin_auth_header, base_credential):
    response = client.post('/api/v2beta/credentials/', headers=admin_auth_header, json=base_credential)
...
def test_update_credential_not_found(client, admin_auth_header, base_credential):
    response = client.put('/api/v2beta/credentials/9999', headers=admin_auth_header, json=base_credential)
...
# todo
def test_update_credential_unique_constraint_failure(client, admin_auth_header, base_credential):
    credential_2 = copy.deepcopy(base_credential)
    credential_2['domain'] = 'the-domain-2'
    response = client.post('/api/v2beta/credentials/', headers=admin_auth_header, json=credential_2)
    assert response.status_code == 201

    response = client.put('/api/v2beta/credentials/2', headers=admin_auth_header, json=base_credential)
...
def test_update_credential(client, admin_auth_header, base_credential):
    ...
    response = client.put('/api/v2beta/credentials/1', headers=admin_auth_header, json=updated_credential)
