...
def test_create_keyword_name_conflict(client, admin_auth_header):
    response = client.post('/api/v2beta/keywords/',
                           headers=admin_auth_header,
                           json={
        'keyword': 'Invoke_Mimikatz',
        'replacement': 'Invoke-Hax'
    })

    assert response.status_code == 400
    assert response.json()['detail'] == 'Keyword with name Invoke_Mimikatz already exists.'
...
def test_create_keyword(client, admin_auth_header):
    response = client.post('/api/v2beta/keywords/',
                           headers=admin_auth_header,
                           json={
        'keyword': 'Invoke-Things',
        'replacement': 'Invoke-sgnihT;'
    })
...
def test_update_keyword_not_found(client, admin_auth_header):
    response = client.put('/api/v2beta/keywords/9999',
                          headers=admin_auth_header,
                          json={
        'keyword': 'thiswontwork',
        'replacement': 'x=0;'
    })
...
def test_update_keyword_name_conflict(client, admin_auth_header):
    response = client.put('/api/v2beta/keywords/1',
                          headers=admin_auth_header,
                          json={
        'keyword': 'Invoke_Mimikatz',
        'replacement': 'Invoke-Whatever'
    })
...
def test_update_keyword(client, admin_auth_header):
    response = client.put('/api/v2beta/keywords/1',
                          headers=admin_auth_header,
                          json={
        'keyword': 'Completely-new_name',
        'replacement': 'qwerefdsgaf'
    })
