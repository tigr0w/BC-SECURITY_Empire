...
def test_get_listener_templates(client, admin_auth_header):
    response = client.get("/api/v2beta/listener-templates/",
...
def test_get_listener_template(client, admin_auth_header):
    response = client.get("/api/v2beta/listener-templates/http",
                          headers=admin_auth_header,)
    assert response.status_code == 200
...
def test_create_listener_validation_fails_required_field(client, base_listener, admin_auth_header):
...
def test_create_listener_custom_validation_fails(client, base_listener, admin_auth_header):
...
def test_create_listener(client, base_listener, admin_auth_header):
    ...
    assert response.json()['options']['DefaultJitter'] == base_listener['options']['DefaultJitter']
    assert response.json()['options']['DefaultDelay'] == base_listener['options']['DefaultDelay']
...
def test_create_listener_name_conflict(client, base_listener, admin_auth_header):
    response = client.post("/api/v2beta/listeners/",
                           ...
                           json=base_listener)
    assert response.status_code == 400
    assert response.json()['detail'] == f"Listener with name {base_listener['name']} already exists."


def test_get_listener(client, admin_auth_header):
    response = client.get(f"/api/v2beta/listeners/{my_globals['listener_id']}",
                          headers=admin_auth_header,)
    assert response.status_code == 200
...
def test_get_listener_not_found(client, admin_auth_header):
    response = client.get("/api/v2beta/listeners/9999",
                          headers=admin_auth_header,)
...
def test_update_listener_not_found(client, base_listener, admin_auth_header):
    ...
    response = client.put("/api/v2beta/listeners/9999",
                          headers=admin_auth_header,
                          json=base_listener)
    assert response.status_code == 404
    assert response.json()['detail'] == 'Listener not found for id 9999'
...
def test_update_listener_blocks_while_enabled(client, admin_auth_header):
    ...
    response = client.put(f"/api/v2beta/listeners/{my_globals['listener_id']}",
                          headers=admin_auth_header,
                          json=response.json())
    assert response.status_code == 400
...
def test_update_listener_name_conflict(client, base_listener, admin_auth_header):
    ...
    response = client.put(f"/api/v2beta/listeners/{created['id']}",
                          headers=admin_auth_header,
                          json=created)
    assert response.status_code == 200
    ...
    assert response.status_code == 400
    assert response.json()['detail'] == "Listener with name new-listener-1 already exists."
...
def test_update_listener_reverts_if_validation_fails(client, admin_auth_header):
    ...
    response = client.put(f"/api/v2beta/listeners/{listener['id']}",
                          headers=admin_auth_header,
                          json=listener)
    assert response.status_code == 400
    assert response.json()['detail'] == 'required listener option missing: Host'

    response = client.get(f"/api/v2beta/listeners/{my_globals['listener_id']}", headers=admin_auth_header)
...
def test_get_listeners(client, admin_auth_header):
    response = client.get(f"/api/v2beta/listeners",
                          headers=admin_auth_header)
...
def test_delete_listener_while_enabled(client, admin_auth_header):
    ...
    to_delete = list(filter(lambda x: x['enabled'] is True, response.json()['records']))[0]
    assert to_delete['enabled'] is True

    response = client.delete(f"/api/v2beta/listeners/{to_delete['id']}",
                             headers=admin_auth_header)
