...
def agent(db):
    ...
    if len(agents) == 0:
        ...
        agent2 = models.Agent(name='SECOND',
                              session_id='SECOND',
                              delay=60,
                              jitter=0.1,
                              external_ip='1.1.1.1',
        ...
        agent3 = models.Agent(name='ARCHIVED',
                              session_id='ARCHIVED',
                              delay=60,
                              jitter=0.1,
                              external_ip='1.1.1.1',
        ...
        agent4 = models.Agent(name='STALE',
                              session_id='STALE',
                              delay=1,
                              jitter=0.1,
                              external_ip='1.1.1.1',
    ...
    main.agents.agents['TEST123'] = {'sessionKey': agents[0].session_key, 'functions': agents[0].functions}
    main.agents.agents['SECOND'] = {'sessionKey': agents[1].session_key, 'functions': agents[1].functions}
    main.agents.agents['ARCHIVED'] = {'sessionKey': agents[2].session_key, 'functions': agents[2].functions}
    main.agents.agents['STALE'] = {'sessionKey': agents[3].session_key, 'functions': agents[3].functions}
...
def test_get_agents_include_stale_false(client, admin_auth_header):
    response = client.get('/api/v2beta/agents?include_stale=false', headers=admin_auth_header)
...
def test_get_agents_include_archived_true(client, admin_auth_header):
    response = client.get('/api/v2beta/agents?include_archived=true', headers=admin_auth_header)
...
def test_update_agent_not_found(client, admin_auth_header):
    ...
    response = client.put('/api/v2beta/agents/XYZ123', json=agent, headers=admin_auth_header)
...
def test_update_agent_name_conflict(client, admin_auth_header):
    ...
    response = client.put('/api/v2beta/agents/TEST123', json=agent, headers=admin_auth_header)
