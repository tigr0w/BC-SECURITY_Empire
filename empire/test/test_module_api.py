...
def test_update_module(client, admin_auth_header):
    uid = 'python_trollsploit_osx_say'
    response = client.put(f'/api/v2beta/modules/{uid}', headers=admin_auth_header, json={
        'enabled': False
    })
...
def test_update_modules_bulk(client, admin_auth_header):
    uids = ['python_trollsploit_osx_say',
            'powershell_code_execution_invoke_boolang',
            'powershell_code_execution_invoke_ironpython',
            ]
    response = client.put(f'/api/v2beta/modules/bulk/enable', headers=admin_auth_header, json={
        'enabled': False,
        'modules': uids,
    })
